...
from .file_util import set_executable, file_size
from .struct_util import serialize
from .log import log
...
def merge_dynamic(to_extend_file, source_file, output_file, base=0x50000000, merge_load_segments=False):
    ...
    #  Many sections have alignment requirements (at least according to llvm-readobj).
    #  We align dynstr so that all subsequent sections will be aligned, as all added apart from the string
    #  tables consists of structs of a fixed size.
    #  We really should align all the sections one by one
    ...
    while matching_segment is not None:
        log(f"Discarding {hex(start_address)} since overlaps the following segment:\n"
    ...
    if merge_load_segments:
        ...
        for s in source_elf.segments:
            ...
            additional_segment_content = b"\x00" * required_padding + additional_segment_content

            additional_segment_phdr.p_offset = additional_segments_offset
            additional_segment_phdr.p_vaddr = additional_segment_phdr.p_vaddr - (
                    additional_segment_phdr.p_vaddr % additional_segment_phdr.p_align)
            additional_segment_phdr.p_paddr = additional_segment_phdr.p_paddr - (
                    additional_segment_phdr.p_paddr % additional_segment_phdr.p_align)
            additional_segments_offset = align(
                additional_segments_offset + required_padding + additional_segment_phdr.p_filesz, 0x1000)
    ...
    # new_segment.p_memsz = estimated_size
    new_segment.p_filesz = new_segment_size
    # new_segment.p_filesz = estimated_size
    new_segment.p_align = alignment

    new_segments += [new_segment]

    # Sort LOAD entries in ascending order
    # TODO: Should the LOAD entry mapping the program headers themselves be first?
    #  Linux uses the first LOAD entry it sees to determine its load_addr.
    #  See https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/fs/binfmt_elf.c#n1146
    new_segments.sort(key=keyer)
